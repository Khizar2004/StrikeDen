...
const ScheduleSchema = new mongoose.Schema({
  ...
  classType: {
    ...
    enum: {
      values: ['Boxing', 'Brazilian Jiu-Jitsu', 'Muay Thai', 'Wrestling', 'MMA', 'Conditioning', 'Kickboxing', 'Taekwondo', 'Judo'],
  ...
  startTimeString: {
    ...
    match: [/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Start time must be in format HH:MM']
  },
  endTimeString: {
    type: String,
    required: [true, 'End time is required'],
    match: [/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'End time must be in format HH:MM']
  },
  trainer: { 
